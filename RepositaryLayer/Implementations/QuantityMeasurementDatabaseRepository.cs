...
namespace RepositoryLayer.Implementations
{
    public class QuantityMeasurementDatabaseRepository : IQuantityMeasurementRepository
    {
        ...
        public void Save(MeasurementRecord record)
        {
            try
            {
                ...
                Console.WriteLine($"  Operand1: {record.Operand1?.Value} {record.Operand1?.Unit}");
                ...
                if (record.Operand2 != null)
                {
                    ...
                    Console.WriteLine($"  Operand2: {record.Operand2.Value} {record.Operand2.Unit}");
                }
                else
                {
                ...
                if (record.Result is MeasurementRequest resultReq)
                {
                    ...
                    Console.WriteLine($"  Result: {resultReq.Value} {resultReq.Unit}");
                }
                else
                {
                ...
                Console.WriteLine($"  ✓ Record saved successfully! Rows affected: {rowsAffected}");
            }
            catch (Exception ex)
            {
        ...
        public IEnumerable<MeasurementRecord> GetAll()
        {
            try
            {
                Console.WriteLine("\n[DEBUG] Retrieving all records from database...");
                ...
                var result = records.Select(r => r.ToMeasurementRecord()).ToList();
                
                Console.WriteLine($"  Retrieved {result.Count} records from database");
                return result;
            }
            catch (Exception ex)
            {
        ...
        public void Clear()
        {
            try
            {
                Console.WriteLine("\n[DEBUG] Clearing all records from database...");
                ...
                Console.WriteLine($"  Deleted {rowsAffected} records from database");
            }
            catch (Exception ex)
            {
    ...
    // Dapper-friendly DTO for database rows
    internal class MeasurementRecordDto
    {
        ...
        public MeasurementRecord ToMeasurementRecord()
        {
            ...
            object result = null;
            if (Result_Value.HasValue)
            {
                result = new MeasurementRequest
                {
            ...
            {
                if (op2 != null)
                    return new MeasurementRecord(Operation, op1, op2, ErrorMessage);
                else
                    return new MeasurementRecord(Operation, op1, ErrorMessage);
            }
            else
            {
                if (op2 != null)
                    return new MeasurementRecord(Operation, op1, op2, result);
                else
                    return new MeasurementRecord(Operation, op1, result);
            }
        }
    }
}